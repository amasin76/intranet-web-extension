...
export type BaseStorage<D> = {
	...
	update: <K extends keyof D>(key: K, value: D[K]) => Promise<void>;
	getSnapshot: () => D | null;
...
export function createStorage<D>(
	...
	const update: <K extends keyof D>(key: K, value: D[K]) => Promise<void> = async (key, value) => {
		await set((currentState) => ({ ...currentState, [key]: value }));
	};
